...
+ (HolySet *)getFromDictionary:(NSDictionary *)jsonData
{
	if(![jsonData isKindOfClass:[NSDictionary class]])
		return nil;
	//Create HolyAuthor object
	HolySet *item = [[HolySet alloc] initWithIdentity:[[jsonData objectForKey:@"id"] integerValue]
...
+ (NSArray *)getAll
{
	...
	cmd.commandText = @"SELECT ID, LanguageID, Name, Image, Desc FROM Sets";
	[conn open];
	
	OSSQLDataReader *reader = [cmd executeReader];
	while ([reader read])
	{
		HolySet *set = [[HolySet alloc] initWithIdentity:[reader getInteger:0]
													   languageID:[reader getInteger:1]
															 name:[reader getString:2]
															image:[reader getString:3]
															 desc:[reader getString:4]];
		[objects addObject:set];
		[set release];
	}
...
+ (HolySet *)getByID: (NSInteger)identity
{
	...
	OSSQLConnection *conn = [[OSSQLConnection alloc] initWithDBPath:[AppHelper databasePath]];
	OSSQLCommand *cmd = [[OSSQLCommand alloc] initWithSQLConnection:conn];
	cmd.commandText = @"SELECT ID, LanguageID, Name, Image, Desc FROM Sets WHERE ID = :ID";
	[conn open];
	...
	if ([reader read])
	{
		set = [[HolySet alloc] initWithIdentity:[reader getInteger:0]
										   languageID:[reader getInteger:1]
												 name:[reader getString:2]
												image:[reader getString:3]
												 desc:[reader getString:4]];
...
+ (BOOL)existsWithID: (NSInteger)identity
{
	...
	OSSQLConnection *conn = [[OSSQLConnection alloc] initWithDBPath:[AppHelper databasePath]];
	OSSQLCommand *cmd = [[OSSQLCommand alloc] initWithSQLConnection:conn];
	cmd.commandText = @"SELECT COUNT(*) FROM Sets WHERE ID = :ID";
	[conn open];
...
- (void)insert
{
	...
	OSSQLCommand *cmd = [[OSSQLCommand alloc] initWithSQLConnection:conn];
	[conn open];
	cmd.commandText = @"INSERT INTO Sets (ID, LanguageID, Name, Image, Desc) VALUES (:ID, :LanguageID, :Name, :Image, :Desc)";
